...
1c. MANDATORY SCHEMA FETCH - Before generating any SQL, you MUST use 
    sql_exec_tool to query INFORMATION_SCHEMA.COLUMNS for the requested 
    source table in Snowflake. 
    - If the table does not exist, STOP and warn the user.
    - If the table exists, retrieve and store the exact list of live columns.
...
2. RULES FOR COLUMN ROUTING:
   ...
   - Staging Layer: Must select EVERY column found in the Snowflake source table.
   - Hub/Link Layer: Must select ONLY the Grain columns (Business Keys), Hash Keys, and audit columns.
   - Satellite Layer: Must select ONLY the descriptive payload columns. It must strictly exclude the Grain columns and BKCC.

3. PRE-EXECUTION SYNC & BRANCH GATE:
...
4. Generate the staging model first, using the Staging template. It
...
7. MANDATORY VALIDATION - before showing any SQL to the user, call
...
8. Show the user: all generated SQL and yml files together, the
...
9. On explicit approval only, write all files via editFiles. Confirm
   each file was created successfully.

10. POST-APPROVAL WAREHOUSE EXECUTION:
